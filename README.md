...
**harness 既是评估性能 gap 的框架，也是优化性能的 loop。** 主线能力是把「量基线 → 找瓶颈 → 提改动 → 对照复量 → 判定保留或回退」这个循环无人值守地跑完，判定规则在看到候选数据**之前**落盘冻结，不达标自动回退且不留痕。底层由 Rust 内核驱动设备（Android adb / OpenHarmony hdc），并提供标准化的运行遥测与状态追溯记录。
...
NN（端侧超分 / 插帧）是这个闭环里的**一类候选改动**，不是主线；算法循环在独立仓库 [`sr_loop`](https://github.com/HGamey/sr_loop)。
...
- **性能优化闭环（`loop_v1/`）**：对一个可重复的游戏负载跑完整一轮「基线 → 归因 → 改动 → 对照 → 统计判定 → 回滚 → 证据归档」。判定用精确置换检验 + 置换反演 CI，规则先于数据冻结；所有 sysfs 写入退出前恢复，进出设备快照逐行相等才算这轮成立；下游解析全是纯函数，同一份 trace 每次重算逐字节一致。
- **帧时序与 GPU 归因**：走 raw ftrace 的 kgsl 事件（不侵入游戏进程），一次采集同时拿到帧节奏、GPU 执行时长、总线带宽投票、热降频。`SurfaceFlinger --latency`、`gfxinfo`、Perfetto GPU producer 三条路都已实测排除，原因见 [`loop_v1/README.md`](loop_v1/README.md)。
