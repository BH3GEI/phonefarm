...
自主 UI 遍历、确定性脚本回放、一致性测试 harness、端侧模型实测标尺、数据采集管线、
设备农场运维、性能优化闭环、假设—实验—证据闭环，以及把上述能力整体外挂给其它 Agent 的 MCP 服务。

多模态视觉模型（VLM）只是其中**一条**上层通路，不是这个项目的定义。
`script` / `test-batch` / `bench` / `capture` / `keepalive` 全程零 Token，不碰模型也能独立工作。
...
## 📌 用在移动游戏性能优化上（[HGamey](https://github.com/HGamey) 场景）

上面说的是这个项目**泛用**的定位。在游戏性能这条具体场景里，它的角色更窄也更明确：

> **harness 既是评估性能 gap 的框架，也是优化性能的 loop。**
> 把「量基线 → 找瓶颈 → 提改动 → 对照复量 → 判定保留或回退」这个循环无人值守地跑完。
> 判定规则在看到候选数据**之前**落盘冻结，不达标自动回退且不留痕。
...
- **黑盒游戏上的天花板是限帧器，不是算力。** 归因 5 轮一致：限帧器封顶 @30fps，GPU 只用掉 64.5%，余量 11.91ms，热事件 0。**任何降低 GPU 工作量的路线（含 NN 超分）在此类负载上都换不出帧率**，只能换功耗与热预算。
- **NN 的账要算端到端。** `bench` 区分 `gpu`（只含模型算子）与 `invoke`（含张量拷贝同步）两个口径，后者高约 **7ms 且与网络结构无关**，540×960→1080×1920 每帧拷贝 **31MB**。端到端约 10ms，吃掉 60fps 帧预算的 60%。这 7ms 是否测量工装产物尚未定论，见路线图 §3。

NN（端侧超分 / 插帧）在这条场景里是闭环的**一类候选改动**，不是主线；算法循环在独立仓库 [`sr_loop`](https://github.com/HGamey/sr_loop)。
...
| 确定性脚本与回放 | `script` | 否 | 固定动作序列执行，或原样回放某次历史对局，保留全套遥测 |
| 一致性测试 harness | `test-batch` `cts-fetch` | 否 | CTS/XTS 批量执行与结果提取，Android/OpenHarmony 双协议，逐用例出断言栈 |
| 端侧模型实测标尺 | `bench` | 否 | TFLite 模型真机延迟：锁频 + 等冷 + GPU Delegate 算子日志解析 |
| 数据采集管线 | `capture` | 否 | 按状态筛选的原始帧采集 + manifest 路线分段 |
| **性能优化闭环** | `loop_v1/` 下的工具链 | 否 | 帧时序采集（raw ftrace + kgsl，不侵入游戏进程）、GPU 归因、系统旋钮、精确置换检验 + 置换反演 CI、自动回滚、证据可字节复现 |
...
对一个**可重复**的游戏负载，无人值守跑完整一轮性能优化：

```
选负载 → 量基线 → 归因 → 改旋钮 → 复量 → 统计判定 → 回滚 → 留证据
```

不侵入游戏进程，所以对第三方黑盒游戏也成立。帧数据走 **raw ftrace 的 kgsl 事件**，
一次采集同时拿到帧节奏、GPU 执行时长、总线带宽投票、热降频。
...
五条判据（2026-09-16 在 NX809J + 原神上全部实测通过）：负载可重复（离散度 + 漂移双查）、
归因跨轮一致、统计显著（精确置换检验 + 置换反演 CI）、不留痕（进出设备快照逐行相等）、
可离线回放（同一份 trace 重算逐字节一致）。
...
| `docs/DESIGN.md` | 核心架构契约：记录契约 v1 / 六步回路 / 任务隔离 / 写入权限 |
...
| **`docs/MOBILE_GPU_OPT_ROUTES.md`** | **移动 GPU 优化技术路线梳理：五类候选 / 每条在 loop_v1 里怎么验 / 黑盒白盒分叉 / NN 的实测代价** |
