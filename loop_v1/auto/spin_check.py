...
与 `tools/frames_moving.py` 的关系
---------------------------------
`tools/frames_moving.py` 是同一件事的**命令行版**, 报的是「平均逐像素绝对差 (%)」;
这里报的是「变化像素占抽样点的比例」。两个统计量不同但互补 —— 前者对变化**幅度**
敏感, 后者对变化**范围**敏感 —— 且对同一对裸帧结论一致: 本闭环第一次原神实跑的
`runs_sysparam/genshin1/spin_check/spin_{a,b}.raw` 上, 前者 13.637%, 后者 81.77%,
都远在各自门槛之上。裸帧头部长度的反推规则两边也是同一条。

留两份是因为职责不同: 那边是给人手查一对帧用的 CLI, 这边是 `autoloop` 里带
通过/不通过门槛、进 `report.json`、有单测覆盖的库函数。要改判据请两边一起改。
