...
| 指标 | 方向 | 来源 |
|---|---|---|
| `frame_p95` | 越小越好 | ftrace kgsl 帧时序 |
| `fps_mean` | 越大越好 | 同上 |
| `power_w_mean` | 越小越好 | `power_supply` 电池轨 |
...
多重比较: 一次看 3 个指标, 都按 0.05 判会把假阳性抬到约 14%。所以「改善」一侧用
Bonferroni 收紧到 `alpha/K`, 「变差」一侧仍用 0.05 不收紧 —— **两侧故意不对称**:
宁可漏掉一个真改善, 也不要把一个真退化放过去。

K=3 时 `alpha_win = 0.0167`, 需要每臂至少 5 轮才够得着 (5v5 置换全枚举 252 种,
最小可能 p = 0.0079)。轮数不足时 `rule.json` 里 `reachable=false` 明写这一条,
任何候选都不可能判保留 —— 规则自己交代它的分辨率上限, 不靠事后解释。

充电时电池轨读到的是流入功率, 量不到整机开销。这种情况 `power_w_mean` 如实报
`None`, 判定自动降到 2 个指标 (`alpha_win=0.025`), 而不是拿一个错的数去比。
