...
> **状态: 可用。** 城市渲染正常, 3 轮基线跑完, frame_p95 离散度 **0.987%**。
>
> 实证(2026-09-23, 红魔 NX809J / Adreno 840 / Vulkan / 风扇开):
>
> | | |
> |---|---|
> | frame_p95 离散度 | **0.987%** (refbench 0.45% / 原神 1.37%) |
> | frame_p50 离散度 | 0.281% |
> | 每帧提交次数 | **1**(自相关自检判定, 原神是 2) |
> | 实体总数 | 303,421–303,427 |
> | 三轮有效性 | 3/3 有效, 零热事件 |
> | 目视 | 摩天楼 / 飞行车流 / 环形建筑 / 霓虹招牌, 无 HUD 无弹窗 |
>
> 离散度落在白盒靶子与闭源真游戏之间 —— 正是"有源码的真游戏"该在的位置。
> 交通等 ECS 系统带来的轮间差异没有把它推到原神那一档, 所以不需要回头按住它们。
...
3 轮 `city_flythrough` / `route_a` / 3600 帧, 12 秒 ftrace 采集窗:

| 指标 | 中位 | 离散度 | 漂移/轮 |
|---|---|---|---|
| `frame_p50` | 33.434 ms | **0.281%** | +0.058% |
| `frame_p95` | 40.308 ms | **0.987%** | +0.393% |
| `frame_mean` | 35.370 ms | 0.964% | -0.482% |
| `gpu_active_mean` | 10.625 ms | 0.896% | -0.360% |
| `fps_mean` | 28.273 | 0.966% | +0.483% |
| `bw_median` | 4950 | 6.061% | 0.000% |

三轮逐轮: p95 = 40.308 / 40.227 / 40.625 ms, 全部 `clean_exit=true`、
`streaming.ready=yes`、零热事件、路线 sha256 一致, 无 `INVALID`。

`bw_median` 的 6.061% 是量化台阶造成的, 不是抖动 —— 三轮取值只有 4950 和 5250
两个档, 漂移恒为 0。判据别拿它当连续量用。

### 测试条件 (影响可比性, 必须同批)

- **红魔内置风扇全程开启**
- **`MEGACITY_COOL_MC=45000`**(45°C)。脚本默认继承 refbench 的 40000, 但这台设备
  被连续测试烤过之后, 风扇下的空闲底温就在 42–44°C, 40°C **根本够不到** ——
  硬等只会让每轮白白多耗 12 分钟的有界轮询然后照跑不误。三轮实际起跑温度
  44.2 / 43.4 / 43.8 °C, 彼此差 0.8°C, 批内可比。
  跨批(尤其和 refbench 的历史数据)比较时要留意这条阈值不同。
...
- [x] ~~复验: 目视确认相机看到的是城市~~ 已确认, 截图在 `/Volumes/USB/megacity/verify2_city.png`
- [x] ~~实测 `render_thread_comm`~~ 确认是 `UnityGfxDeviceW`, 已固化进 contract
- [x] ~~3 轮基线~~ frame_p95 离散度 **0.987%**, 落在 refbench 0.45% 与原神 1.37% 之间
- [ ] 接进 loop_v1 的编排(目前只有单轮驱动面, 还没做旋钮与两臂对照)
