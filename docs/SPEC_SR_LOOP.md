...
> **Gate 3 已通过** (gen_1.json: LLM 变异 6/6 真机可行, 零错误, 最佳 +2.21 dB),
> **Gate 4 已结项** (13 代, 用户 2026-09-09 决定提前终止; 累计最优 Spearman rho = 0.966, p = 7.7e-8, 结论"显著正向演进"; 见 §7)
...
全部五道门禁在 NX809J (SM8850 / Adreno 840, Magisk root) 上由程序判定通过; 证据在 sr_loop 仓库 `runs/gate0..2/`, `runs/gate1/`,
`runs/evo/`。Gate 4 原定 20 代, 用户在第 13 代归档后决定提前结项 (前 12 代统计已极显著, 且第 10 代已拿到真机 SLOW 否决反例)。

| 项 | 结果 |
|---|---|
| 演进代数 | 13 代 (gen 1..13), 每代 6 个 LLM 子代, 全部零人工介入; 第 2 代 LLM 通道瞬时空回包, 按设计随机补齐 (已加重试) |
| 累计最优 PSNR 序列 | Spearman rho = 0.966, p = 7.7e-8 (n = 13): 39.498 → 39.862 dB |
| 逐代最优 PSNR 序列 | Spearman rho = 0.747, p = 0.0033 |
| 历史最优个体 | gen10-6: GPU 内核 2.967 ms, val PSNR 39.862 dB (+2.576 dB vs Bicubic 37.286; +1.04 dB vs Gen 0 最佳 38.823) |
| 物理约束生效 | 第 10 代 gen10-5 真机 4.151 ms > 4.0 ms → SLOW 否决; 第 1 代起 skip=none 的个体持续落后 4~9 dB, 变异器随代际收敛到带跳连结构 |
| 归档 | 80 个基因组 (指纹去重), Pareto 前沿 12 个 (延迟 1.28~3.94 ms) |

结论: 在固定数据集、固定短训预算与真机延迟门禁下, LLM 结构变异 + 零样本真机秒筛 + Pareto 归档的闭环能推动超分小网络正向演进,
且演进受物理约束 (延迟门禁、FLOPs 预算) 实际塑形。局限: 13 代样本量下 +0.36 dB 的累计增益较小; 短训 2000 步的排名与充分训练的排名是否一致未验证。
