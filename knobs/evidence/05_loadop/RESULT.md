...
- 负载 `bw_pingpong`，`frames=1800`（实际提交 3600 帧 = 每帧 2 次提交），`intensity=8`
...
|---|---|---|---|---|---|---|---|
| A | 无 | off | LOAD + DONT_CARE | — | — | — | 3600 / true |
| B | 无 | on  | 只有 DONT_CARE | — | — | — | 3600 / true |
| C | **改写** | off | LOAD + DONT_CARE | pass 2 att 0 LOAD→DONT_CARE | **28800** | null | 3600 / true |
| D | **改写** | on  | 只有 DONT_CARE | pass 2 att 0 LOAD→DONT_CARE | **0** | `rewritten pass never bound` | 3600 / true |
...
2. **改的那个 pass 真被用上了**：`begins = 28800 = 3600 帧 × 8 (intensity)`，
   与 `render_pass_begins = 32400`（= 28800 + 3600 个 present blit）自洽。
3. **应用不知道自己被改了**：refbench 仍自报 `load_op: "LOAD"`。
   "应用以为是 LOAD、驱动实际收到 DONT_CARE" 就是灰档改写生效的定义。
4. **没把应用弄坏**：四个臂全部 `frames_submitted=3600`、`clean_exit=true`，与无层基线相同。
