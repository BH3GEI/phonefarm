...
**前置探针已过**（2026-09-23，[`evidence/08_copyprobe/`](../evidence/08_copyprobe/RESULT.md)）：
`debug.knobs.copyprobe=1` 把建 pipeline / 建 image / 注入 dispatch / 克隆替换描述符
全链路以 1:1 拷贝算子每帧走通，5 臂全部跑满一轮、无反作弊拦截、帧时无损
（p50 16.64–16.67ms vs 无层 16.66–16.68ms）。目标绑定是 **binding 0**。
**真算子已投**（2026-09-23，[`evidence/09_upop/`](../evidence/09_upop/RESULT.md)）：
game_opt_loop 真实帧优胜者 gen1_loc3（2×2 bilinear，imageLoad 改写为数学逐位等价的
texelFetch）装进层，渲染精度·低 档下替换游戏自带放大。5 对 A/B 按 loop_v1 冻结口径
判定 **无指标显著变差**（p95 −0.19%、fps +0.15%、功耗 +4.1% 但 p_lose=0.28 属噪声、
37°C 平直）；画质上所有测区 MAD 比游戏自带放大接近原生 3–4 倍。渲染精度已还原极高
（设置改动必须优雅退出才落盘，已复核 2140×968 pass 回归）。
...
1. ~~超分算子：探针已过，直接投真算子~~ ✅ 已投（见上，09_upop）。后续是算子迭代：
   2×2 bilinear 只是占位优胜者，换 game_opt_loop 后续赛道更强的算子（同契约换装即可）。
